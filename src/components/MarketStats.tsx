...
export function MarketStats({ onRefresh, lastCandle, loading: externalLoading }: MarketStatsProps) {
  ...
  const displayPrice = lastCandle?.close ?? data?.price;
  const isPositive = data?.priceChangePercent24h !== undefined && data.priceChangePercent24h >= 0;
      ...
      {displayPrice && !isLoading && (
        <>
          <div className="price-display">
            ...
            <span className="price">{displayPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
